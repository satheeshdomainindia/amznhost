{if $invalid}
    ...
    {include file="$template/includes/alert.tpl" type="error" msg=$LANG.clientareacancelinvalid textcenter=false}
    <p class="text-left">
        <a href="clientarea.php?action=productdetails&amp;id={$id}" class="btn primary-solid-btn">{$LANG.clientareabacklink}</a>
    </p>

{elseif $requested}

    {include file="$template/includes/alert.tpl" type="success" msg=$LANG.clientareacancelconfirmation textcenter=false}
...
{else}
    ...
    {include file="$template/includes/alert.tpl" type="info" textcenter=false msg="{$LANG.clientareacancelproduct}: <strong>{$groupname} - {$productname}</strong>{if $domain} ({$domain}){/if}"}
    ...
    <form method="post" action="{$smarty.server.PHP_SELF}?action=cancel&amp;id={$id}" class="form-stacked">
        ...
        <fieldset>
            ...
            <div class="form-group">
                <div class="form-inline">
                    <input type="submit" value="{$LANG.clientareacancelrequestbutton}" class="btn delete-btn" />
                    <a href="clientarea.php?action=productdetails&id={$id}" class="btn outline-btn">{$LANG.cancel}</a>
                </div>
            </div>
